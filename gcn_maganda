...
if __name__ == "__main__":
    ...
    def _ensure_augmented_cache():
        ...
        out_X = os.path.join(AUGMENTED_DIR, "subject_embedding_matrices_lesswindow.npy")
        ...
        if os.path.exists(out_X) and os.path.exists(out_A) and os.path.exists(out_y):
            ...
            try:
                ...
                return
        in_X = os.path.join(INPUT_DIR, "subject_embedding_matrices_lesswindow.npy")
    ...
    # -------- load your data --------
    X_np = np.load(os.path.join(DATA_DIR, "subject_embedding_matrices_lesswindow.npy"))   # (S, N, 128)
    ...
    model = GraphLevelGCN(
        ...
        gcn_dropout=0.5, gcn_input_dropout=0.1, use_batchnorm=True, graph_dropout=0.5
    ).to(device)
